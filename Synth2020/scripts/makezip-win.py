...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\Synth2020 Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\Synth2020 Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\Synth2020 manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\Synth2020.vst3\Contents\\x86_64-win\Synth2020.vst3",
      projectpath + "\\build-win\Synth2020_x64.exe"  
    ]

  zipname = "Synth2020-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Synth2020-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Synth2020-app_x64.pdb"  
  ]
